...
interface TraitBadgesProps {
  ...
  iconSize?: number;
  /** Distance between the start of one icon and the next. Default: 40 */
  spacing?: number;
}
...
const TraitIcon = ({ trait, index, size, spacing }: { trait: string; index: number; size: number; spacing: number }) => {
  // Assumes assets are served from public/assets/icons/
  // e.g. 'Sword' -> '/assets/icons/Sword.png'
  const [image] = useImage(TRAIT_ICONS[trait] || `/assets/icons/${trait}.png`);
