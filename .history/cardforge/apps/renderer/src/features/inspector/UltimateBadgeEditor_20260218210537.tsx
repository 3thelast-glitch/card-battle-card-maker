...
export interface BadgeElement {
  ...
  zIndex: number;
  customImage?: string;
  animation?: 'none' | 'pulse' | 'spin' | 'float' | 'wiggle' | 'glow';
  animationDuration?: number;
}

// Alias for compatibility with request
...
export interface BadgePreset {
  ...
  layout: 'horizontal' | 'vertical' | 'grid';
  animation: 'pulse' | 'glow' | 'spin' | 'float' | 'wiggle' | 'none';
}
...
export function UltimateBadgeEditor({ badges, onUpdate }: UltimateBadgeEditorProps) {
  ...
  const handlePresetApply = (preset: BadgePreset) => {
    ...
    updateBadgeWithHistory(selectedId, {
      ...
      // Apply theme defaults for effects based on preset
      gradient: true,
      shadowIntensity: 0.5,
      glow: 10,
    });
...
interface BadgeStylingPanelProps {
  ...
  availablePresets: BadgePreset[];
  onDelete?: () => void;
  onDuplicate?: () => void;
}

function BadgeStylingPanel({ badge, onChange, availablePresets, onDelete, onDuplicate }: BadgeStylingPanelProps) {
    ...
    <div className="flex-1 flex flex-col min-h-0 bg-slate-50/50">
            ...
            <Section label="Presets">
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                {availablePresets.map(preset => (
                  <button
                    key={preset.id}
                    onClick={() => onChange({ color: preset.colors.primary, color2: preset.colors.secondary, glowColor: preset.colors.glow, layout: preset.layout, animation: preset.animation as any })}
                  ...
                  >
                    <div className="w-6 h-6 rounded-full shadow-sm transition-transform group-hover:scale-110" style={{ background: `linear-gradient(135deg, ${preset.colors.primary}, ${preset.colors.secondary})`, boxShadow: `0 0 8px ${preset.colors.glow}` }} />
      ...
      <div className="p-3 border-t border-slate-100 bg-white space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 pt-1">
          {onDuplicate && <button onClick={onDuplicate} className="flex items-center justify-center gap-2 px-3 py-1.5 text-xs font-medium text-slate-600 bg-slate-50 border border-slate-200 hover:bg-slate-100 rounded transition-colors"><Copy className="w-3 h-3" /> Duplicate</button>}
          {onDelete && <button onClick={onDelete} className="flex items-center justify-center gap-2 px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 hover:bg-red-100 rounded transition-colors"><Trash2 className="w-3 h-3" /> Delete</button>}
        </div>
      </div>
    </div>
  );
