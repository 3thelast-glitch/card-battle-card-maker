...
export const BadgeStylingPanel = memo(({ badge, onChange }: BadgeStylingPanelProps) => {
  ...
  const handleToggleDarkMode = useCallback(() => {
    ...
    console.log(`🌙 تغيير الوضع إلى: ${newMode ? 'الليلي' : 'النهاري'}`);
  }, [isDarkMode]);

  // دالة لإعادة تعيين جميع الإعدادات
  const handleReset = useCallback(() => {
    console.log('🔄 إعادة تعيين جميع الإعدادات');
    const resetBadge: Partial<BadgeStyle> = {
      color: '#ffffff',
      gradient: true,
      shadowIntensity: 0,
      glow: 0,
  ...
  // دالة لتغيير النمط المسبق مع أنواع صارمة
  const handlePresetChange = useCallback((preset: { name: string; style: Partial<BadgeStyle> }) => {
    console.log(`⚙️ تطبيق النمط المسبق: ${preset.name}`);
    onChange({ ...b, ...preset.style });
  }, [b, onChange]);

  // دالة لتغيير النمط الخاص مع أنواع صارمة
  const handleSpecialStyleChange = useCallback((specialStyle: { name: string; style: Partial<BadgeStyle> }) => {
  ...
  const handleSaveChanges = useCallback(() => {
    ...
    console.log('📊 البيانات المحفوظة:', b);
