...
export function CardInspector(props: {
  ...
  const { row, project, language, onUpdateData, onUpdateStat, onUpdateRow, onPickImage } = props;
  ...
  const assetOptions = useMemo(
    ...
    [projectRoot, project.assets?.images],
  );
  ...
  const addTrait = (trait: TraitKey) => {
    ...
    onUpdateData('traits', [...traits, trait]);
  };

  const removeTrait = (trait: TraitKey) => {
    onUpdateData('traits', traits.filter((t) => t !== trait));
  };
        ...
        <div className="uiAccordionBody uiStack">
          <Row gap={10}>
            <div className="flex-1">
              ...
              <Input value={nameEn} onChange={(e) => onUpdateData('`name.en`', e.target.value)} />
            </div>
            <div className="flex-1">
              <div className="uiHelp">{t('`common.name`')} ({t('`settings.arabic`')})</div>
              <Input value={nameAr} onChange={(e) => onUpdateData('`name.ar`', e.target.value)} />
            </div>
          </Row>
          <Row gap={10}>
            <div>
              ...
              <Input
                type="number"
                value={cost}
                onChange={(e) => onUpdateData('cost', Number(e.target.value) || 0)}
              />
            </div>
            <div className="flex-1">
              ...
              <Select
                value={element}
                onChange={(e) => onUpdateData('element', e.target.value || undefined)}
              >
          ...
          <div>
            ...
            <Select
              value={race}
              onChange={(e) => onUpdateData('race', e.target.value || undefined)}
            >
              <option value="">{t('`common.none`')}</option>
          ...
          <Row gap={10}>
            <div className="flex-1">
              ...
              <Input
                type="number"
                value={attack}
                onChange={(e) => onUpdateStat('attack', Number(e.target.value) || 0)}
              />
            </div>
            <div className="flex-1">
              ...
              <Input
                type="number"
                value={defense}
                onChange={(e) => onUpdateStat('defense', Number(e.target.value) || 0)}
              />
            </div>
          </Row>
        </div>
        ...
        <div className="uiAccordionBody uiStack">
          <Row gap={10}>
            <div className="flex-1">
              ...
              <Input value={descriptionEn} onChange={(e) => onUpdateData('`desc.en`', e.target.value)} />
            </div>
            <div className="flex-1">
              <div className="uiHelp">{t('`common.description`')} ({t('`settings.arabic`')})</div>
              <Input value={descriptionAr} onChange={(e) => onUpdateData('`desc.ar`', e.target.value)} />
            </div>
          </Row>
          <Row gap={10}>
            <div className="flex-1">
              <div className="uiHelp">{t('`editor.inspector.ability`', { defaultValue: 'Ability Text' })} ({t('`settings.english`')})</div>
              <Input value={abilityEn} onChange={(e) => onUpdateData('`ability.en`', e.target.value)} />
            </div>
            <div className="flex-1">
              <div className="uiHelp">{t('`editor.inspector.ability`', { defaultValue: 'Ability Text' })} ({t('`settings.arabic`')})</div>
              <Input value={abilityAr} onChange={(e) => onUpdateData('`ability.ar`', e.target.value)} />
            </div>
          </Row>
        </div>
      </details>
        ...
        <div className="uiAccordionBody uiStack">
          <div>
            ...
            <TemplatePicker
              value={templateKey}
              language={language}
              onChange={(next) => onUpdateData('templateKey', next)}
            />
          </div>
          <div>
            ...
            <Select
              value={rarity}
              onChange={(e) => onUpdateData('rarity', e.target.value)}
            >
                ...
                {assetOptions.map((asset) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => onUpdateRow({ art: { kind: 'image', src: asset.resolvedSrc } })}
                    >
                      <div
