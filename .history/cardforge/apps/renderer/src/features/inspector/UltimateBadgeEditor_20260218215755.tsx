...
import { BadgeStylingPanel } from './BadgeStylingPanel';
import { 
...
const PRESETS: BadgeStylePreset[] = [
  ...
  {
    id: 'gf-legendary', name: 'Golden Legend', category: 'Fantasy', previewColor: '#D4AF37',
    colors: { color: '#D4AF37', color2: '#FFD700' },
    effects: { gradient: true, gradientType: 'linear', gradientAngle: 135, shadowIntensity: 0.8, glow: 15, borderWidth: 2, borderColor: '#FFF8DC', animation: 'float', animationDuration: 3 }
  },
  {
    id: 'gf-epic', name: 'Epic Void', category: 'Fantasy', previewColor: '#8A2BE2',
    colors: { color: '#8A2BE2', color2: '#4B0082' },
    effects: { gradient: true, gradientType: 'radial', shadowIntensity: 0.6, glow: 10, borderWidth: 1, borderColor: '#E6E6FA', animation: 'pulse', animationDuration: 2 }
  },
  {
    id: 'gf-rare', name: 'Rare Essence', category: 'Fantasy', previewColor: '#4169E1',
    colors: { color: '#4169E1', color2: '#1E90FF' },
    effects: { gradient: true, gradientType: 'linear', gradientAngle: 45, shadowIntensity: 0.4, glow: 5, borderWidth: 1, borderColor: '#87CEFA' }
  },
  {
    id: 'gf-common', name: 'Iron Common', category: 'Fantasy', previewColor: '#A0A0A0',
    colors: { color: '#A0A0A0', color2: '#C0C0C0' },
    effects: { gradient: false, shadowIntensity: 0.3, borderWidth: 1, borderColor: '#696969' }
  },
  // --- Existing Presets ---
  { 
    id: 'neon-blue', name: 'Neon Blue', category: 'Neon', previewColor: '#00f3ff',
    colors: { color: '#00f3ff', color2: '#0066ff' }, 
    effects: { borderWidth: 2, shadowIntensity: 0.8, opacity: 1, backgroundOpacity: 0.2, glow: 8, gradient: true } 
  },
  { 
    id: 'neon-pink', name: 'Neon Pink', category: 'Neon', previewColor: '#ff00ff',
    colors: { color: '#ff00ff', color2: '#cc00cc' }, 
    effects: { borderWidth: 2, shadowIntensity: 0.8, opacity: 1, backgroundOpacity: 0.2, glow: 8, gradient: true } 
  },
  { 
    id: 'dark-matter', name: 'Dark Matter', category: 'Dark', previewColor: '#1a1a1a',
    colors: { color: '#1a1a1a' }, 
    effects: { borderWidth: 0, shadowIntensity: 0.9, opacity: 1, backgroundOpacity: 0.9, borderRadius: 50 } 
  },
  { 
    id: 'obsidian', name: 'Obsidian', category: 'Dark', previewColor: '#000000',
    colors: { color: '#000000' }, 
    effects: { borderWidth: 1, shadowIntensity: 0.5, opacity: 0.9, backgroundOpacity: 0.8, borderStyle: 'solid' } 
  },
  { 
    id: 'glass-white', name: 'Glass', category: 'Minimal', previewColor: '#ffffff',
    colors: { color: '#ffffff' }, 
    effects: { borderWidth: 1, shadowIntensity: 0.2, opacity: 0.9, backgroundOpacity: 0.1, borderRadius: 12 } 
  },
  { 
    id: 'frosted', name: 'Frosted', category: 'Minimal', previewColor: '#e2e8f0',
    colors: { color: '#e2e8f0' }, 
    effects: { borderWidth: 0, shadowIntensity: 0.1, opacity: 0.8, backgroundOpacity: 0.3, borderRadius: 8 } 
  },
  { 
    id: 'gold-rush', name: 'Gold Rush', category: 'Metallic', previewColor: '#ffd700',
    colors: { color: '#ffd700', color2: '#b8860b' }, 
    effects: { borderWidth: 2, shadowIntensity: 0.5, opacity: 1, backgroundOpacity: 0.3, gradient: true, borderStyle: 'double' } 
  },
  { 
    id: 'silver-lining', name: 'Silver', category: 'Metallic', previewColor: '#c0c0c0',
    colors: { color: '#c0c0c0', color2: '#e0e0e0' }, 
    effects: { borderWidth: 1, shadowIntensity: 0.4, opacity: 1, backgroundOpacity: 0.2, gradient: true } 
  },
  { 
    id: 'fire-gradient', name: 'Inferno', category: 'Gradient', previewColor: '#ff4500',
    colors: { color: '#ff4500', color2: '#ff8c00' }, 
    effects: { borderWidth: 0, shadowIntensity: 0.6, opacity: 1, backgroundOpacity: 0.8, gradient: true, borderRadius: 50 } 
  },
  { 
    id: 'ocean-depths', name: 'Ocean', category: 'Gradient', previewColor: '#00bfff',
    colors: { color: '#00bfff', color2: '#1e90ff' }, 
    effects: { borderWidth: 0, shadowIntensity: 0.5, opacity: 0.9, backgroundOpacity: 0.8, gradient: true, borderRadius: 50 } 
  },
  { 
    id: 'nature-green', name: 'Nature', category: 'Flat', previewColor: '#32cd32',
    colors: { color: '#32cd32' }, 
    effects: { borderWidth: 0, shadowIntensity: 0.2, opacity: 1, backgroundOpacity: 1, borderRadius: 4 } 
  },
  { 
    id: 'royal-purple', name: 'Royal', category: 'Flat', previewColor: '#8a2be2',
    colors: { color: '#8a2be2' }, 
    effects: { borderWidth: 0, shadowIntensity: 0.3, opacity: 1, backgroundOpacity: 1, borderRadius: 4 } 
  },
];
              ...
              >
                <div 
                  ...
                  style={{
                    ...
                    filter: badge.glow ? `drop-shadow(0 0 ${badge.glow}px ${badge.color})` : undefined,
                  }}
                >
...
interface BadgeStylingPanelProps {
  ...
  onDelete: () => void;
  onDuplicate: () => void;
  onPresetApply: (preset: BadgeStylePreset) => void;
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
          ...
          <>
            ...
            <Section label="Colors">
              <div className="space-y-3">
                ...
                <HexColorPicker color={badge.color} onChange={(c) => onUpdate({ color: c })} style={{ width: '100%', height: '100px' }} />
                ...
                {badge.gradient && (
                  <>
                    ...
                    <HexColorPicker color={badge.color2 || badge.color} onChange={(c) => onUpdate({ color2: c })} style={{ width: '100%', height: '100px' }} />
        ...
        {activeTab === 'effects' && (
          <>
            <Section label="Dimensions">
              <ControlRow label="Scale" value={`${badge.scale.toFixed(1)}x`} icon={<Maximize className="w-3 h-3" />}>
                <input type="range" min="0.4" max="2.5" step="0.1" value={badge.scale} onChange={e => onUpdate({ scale: +e.target.value })} className="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600" />
              </ControlRow>
            ...
            <Section label="Border & Shadow">
              ...
              <ControlRow label="Shadow" value={`${Math.round(badge.shadowIntensity * 100)}%`}>
                <input type="range" min="0" max="1" step="0.1" value={badge.shadowIntensity} onChange={e => onUpdate({ shadowIntensity: +e.target.value })} className="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600" />
              </ControlRow>
        ...
        {activeTab === 'anim' && (
          <Section label="Animation Timeline">
            ...
            {badge.animation && badge.animation !== 'none' && (
              <ControlRow label="Duration" value={`${badge.animationDuration || 2}s`}>
                <input type="range" min="0.5" max="10" step="0.5" value={badge.animationDuration || 2} onChange={e => onUpdate({ animationDuration: +e.target.value })} className="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-purple-600" />
              </ControlRow>
            )}
          </Section>
        )}
              ...
              <div className="grid grid-cols-2 gap-2">
                {PRESETS.map(preset => (
                  <button
                    ...
                    className="text-left p-2 rounded border border-slate-200 hover:border-blue-300 bg-white text-xs"
                  >
                    <div className="flex items-center gap-2">
              ...
              <div className="space-y-1">
                {[...allBadges].reverse().map((b, i) => (
                  <div key={b.id} onClick={() => onSelect(b.id)} className={`flex items-center justify-between p-2 rounded border cursor-pointer ${b.id === badge.id ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white'}`}>
                    <div className="flex items-center gap-2">
                      <BadgeIconSmall badge={b} />
                      <span className="text-xs font-medium truncate w-24">{b.name}</span>
                    </div>
                    {b.id === badge.id && (
                      <div className="flex gap-1">
      ...
      <div className="p-4 border-t border-slate-100 bg-white grid grid-cols-2 gap-2">
        <button onClick={onDuplicate} className="flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-slate-600 bg-slate-50 border border-slate-200 hover:bg-slate-100 rounded-lg transition-colors"><Copy className="w-4 h-4" /> Duplicate</button>
        <button onClick={onDelete} className="flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /> Delete</button>
      </div>
    </div>
  );
}
