...
export const BadgeStylingPanel = memo(({ badge, onChange, onDelete, onDuplicate }: BadgeStylingPanelProps) => {
  ...
  const handlePresetSelect = (preset: BadgePreset) => {
    onChange({
      ...
      color2: preset.secondary || preset.primary,
      glowColor: preset.glow,
      // Default values since preset structure changed
      layout: 'vertical',
      animation: 'pulse',
      gradient: true,
      shadowIntensity: 0.5,
          ...
          <>
            ...
            <Section label="Colors">
              <div className="space-y-3">
                ...
                <input type="color" value={badge.color} onChange={(e) => onChange({ color: e.target.value })} className="w-full h-10 rounded cursor-pointer border border-slate-200 p-0.5" />
                ...
                {badge.gradient && (
                  <>
                    ...
                    <input type="color" value={badge.color2 || badge.color} onChange={(e) => onChange({ color2: e.target.value })} className="w-full h-10 rounded cursor-pointer border border-slate-200 p-0.5" />
        ...
        {activeTab === 'effects' && (
          <>
            ...
            <Section label="Border & Shadow">
              ...
              <ControlRow label="Glow" value={`${badge.glow || 0}px`} icon={<Icons.Sun />}>
