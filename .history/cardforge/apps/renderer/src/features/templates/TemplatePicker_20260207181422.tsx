...
export function TemplatePicker({ value, language, onChange }: Props) {
  ...
  const descriptionFor = (key: TemplateKey, customKey?: string) =>
    t(customKey ?? `templates.${key}.desc`, { defaultValue: t('templates.descriptionFallback') });
      ...
      <button type="button" className="templatePickerField" onClick={() => setOpen(true)}>
        ...
        <div className="templatePickerMeta">
          <div className="templatePickerTitle">
            {selected
              ? titleFor(selected.key, selected.label[language] ?? selected.label.en, selected.titleKey)
              : ''}
          </div>
                  ...
                  {filtered.map((template) => (
                    ...
                    >
                      <div
                        ...
                        style={thumbStyle(template.defaultBgColor, template.thumbnail)}
                      />
                      {template.key === value ? <span className="templateCardCheck">✓</span> : null}
                      <div className="templateCardMeta">
                        ...
                        <div className="templateCardDesc">{descriptionFor(template.key, template.descKey)}</div>
                      </div>
                    </div>
                  ))}
                </div>
