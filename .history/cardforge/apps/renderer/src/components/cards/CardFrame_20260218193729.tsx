...
type BadgeStyle = {
  ...
  borderRadius?: number;
  shadow?: string; // 'none' | 'soft' | 'strong' | 'neon'
  glow?: number;
};
...
export function CardFrame({
  ...
  const tribeCss = getBadgeCss(tribeBadge);
  ...
  const attackCss = getBadgeCss(attackBadge);
  const attackStyle = {
    ...buildStatStyle(
  ...
  const defenseCss = getBadgeCss(defenseBadge);
  const defenseStyle = {
    ...buildStatStyle(
      defenseBadge.color || tint,
      isLegendary,
      isEpic,
      defenseBadge.scale ?? 1,
      'right bottom',
      defenseBadge.color,
    ),
    ...defenseCss.style,
  ...
  const elementCss = getBadgeCss(elementBadge);
...
function normalizeBadgeStyle(style?: BadgeStyle): Required<BadgeStyle> {
  ...
  const color2 = String(style?.color2 ?? '').trim();
  const gradient = Boolean(style?.gradient);
  ...
  const shadow = String(style?.shadow ?? 'none');
...
function BadgeIcon({ iconId }: { iconId: string }) {
