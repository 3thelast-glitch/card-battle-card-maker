...
import { BadgeModel } from './badge.types';
...
export const LiveBadgePreview = memo(
  ({ badge, selected, onClick, style, className }: LiveBadgePreviewProps) => {
    ...
    const badgeStyle: React.CSSProperties = {
      ...
      filter: badge.glow
        ? `drop-shadow(0 0 ${badge.glow}px ${badge.glowColor || badge.color})`
        : undefined,
        ...
        >
          {badge.type === 'number' ? (
            <span
              ...
              style={{ color: badge.color === '#ffffff' ? '#000' : '#fff' }}
            >
              {badge.text || '0'}
            </span>
          ) : (
