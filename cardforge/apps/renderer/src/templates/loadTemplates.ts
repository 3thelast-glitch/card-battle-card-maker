...
export function loadBuiltInTemplates(): TemplateDefinition[] {
  ...
  const templates: TemplateDefinition[] = Object.values(modules).map(
    (mod: any) => {
      ...
      return {
        ...
        thumbnail:
          tpl.thumbnail ?? '/assets/backgrounds/template-placeholder.svg',
      };
    },
  );
  return templates.sort((a, b) =>
    getTemplateSortName(a).localeCompare(getTemplateSortName(b)),
  );
}
