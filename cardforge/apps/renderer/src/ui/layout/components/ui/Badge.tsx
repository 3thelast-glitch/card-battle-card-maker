...
const RARITY_LABELS: Record<Rarity, string> = {
    Common: 'عادي', Uncommon: 'غير شائع', Rare: 'نادر', Epic: 'ملحمي', Legendary: 'أسطوري',
};

const SIZES = { sm: 'text-[9px] px-2 py-0.5', md: 'text-xs px-2.5 py-1', lg: 'text-sm px-3 py-1' };

export const RarityBadge = memo<RarityBadgeProps>(({ rarity, size = 'sm' }) => (
    <span className={`inline-flex items-center font-bold rounded-full border uppercase tracking-wider ${RARITY_STYLES[rarity]} ${SIZES[size]}`}>
...
const ELEMENT_STYLES: Record<string, { bg: string; border: string; emoji: string }> = {
    fire: { bg: 'bg-red-500/20', border: 'border-red-500/40', emoji: '🔥' },
    water: { bg: 'bg-sky-500/20', border: 'border-sky-500/40', emoji: '💧' },
    nature: { bg: 'bg-emerald-500/20', border: 'border-emerald-500/40', emoji: '🌿' },
    dark: { bg: 'bg-purple-500/20', border: 'border-purple-500/40', emoji: '🌑' },
    light: { bg: 'bg-amber-500/20', border: 'border-amber-500/40', emoji: '✨' },
    neutral: { bg: 'bg-slate-500/20', border: 'border-slate-500/40', emoji: '⚪' },
};

export const ElementBadge = memo<ElementBadgeProps>(({ element, size = 'sm' }) => {
    const cfg = ELEMENT_STYLES[element?.toLowerCase()] ?? ELEMENT_STYLES.neutral;
    return (
        <span className={`inline-flex items-center gap-1 font-medium rounded-full border ${cfg.bg} ${cfg.border} ${size === 'sm' ? 'text-[9px] px-2 py-0.5' : 'text-xs px-2.5 py-1'}`}>
            <span>{cfg.emoji}</span>
