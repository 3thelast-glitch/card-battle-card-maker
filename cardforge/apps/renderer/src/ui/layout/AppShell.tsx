...
const EditorScreen = lazy(() =>
  import('../../features/editor/EditorScreen').then((m) => ({
    default: m.EditorScreen,
  })),
);
const ExportScreen = lazy(() =>
  import('../../features/export/ExportScreen').then((m) => ({
    default: m.ExportScreen,
  })),
);
const DataTableScreen = lazy(() =>
  import('../../features/data/DataTableScreen').then((m) => ({
    default: m.DataTableScreen,
  })),
);
const AssetsScreen = lazy(() =>
  import('../../features/assets/AssetsScreen').then((m) => ({
...
export const AppShell = () => {
  ...
  // ── Global state ──
  const { project, setProject, setRecents, updateProject } = useAppStore();
  ...
  const handleOpen = useCallback(async () => {
    ...
    try {
      ...
      const projWithPath = {
        ...proj,
        meta: { ...proj.meta, filePath: res.filePath },
      };
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const savedProject = {
        ...project,
        meta: { ...project.meta, filePath: filePath! },
      };
  ...
  const LEGACY_TOOLS = [
    {
      ...
      icon: Cpu,
      label: 'محرر Konva',
      hint: 'تحرير عناصر البطاقة',
    },
    {
      id: 'data' as Legacy,
      icon: Database,
      label: 'جداول البيانات',
      hint: 'استيراد CSV / Excel',
    },
    {
      id: 'export' as Legacy,
      icon: Download,
  ...
  return (
    ...
    >
      ...
      {/* ══════════════════════════════════════════════════════ */}
      <header
        className="h-14 flex-shrink-0 flex items-center px-4 z-50
      ...
      >
        <div className="w-full flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <div
              className="w-9 h-9 bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-600
            ...
            <div className="hidden sm:block leading-none">
              <h1
                className="text-lg font-black bg-gradient-to-r from-purple-400 via-indigo-400 to-blue-400
          ...
          {/* Main nav */}
          <nav
            className="flex items-center bg-slate-900/60 backdrop-blur-xl px-1 py-1
            ...
            {NAV_ITEMS.map(({ id, icon: Icon, label, badge }) => {
              ...
              return (
                ...
                >
                  ...
                  {badge && (
                    <span
                      className="absolute -top-2 -right-2 px-1.5 py-0.5 min-w-[20px]
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {/* Classic Tools */}
            <button
              onClick={() => setLegacy('editor')}
              title="الأدوات الكلاسيكية"
              className="p-2 rounded-xl bg-white/[0.05] border border-white/[0.08]
      ...
      {legacy !== null && (
        <div className="fixed inset-0 z-[200] flex items-stretch bg-black/70 backdrop-blur-sm">
          {/* Sidebar */}
          <aside
            className="w-48 flex-shrink-0 bg-[#0b0f1a] border-r border-white/[0.07]
                ...
                {legacy === 'editor' && !project && (
                  <div className="h-full flex items-center justify-center flex-col gap-3 text-center">
                    ...
                    <p className="text-sm text-slate-500">
                      افتح مشروعًا أولاً لتشغيل محرر Konva
                    </p>
                    <button
                      onClick={handleOpen}
