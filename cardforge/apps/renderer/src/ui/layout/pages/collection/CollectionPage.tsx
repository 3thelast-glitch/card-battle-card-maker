...
import { Search, Filter, Grid3X3, List, Download, Trash2, CheckSquare, Square, Archive, SortAsc } from 'lucide-react';
...
const ELEMENTS_ALL: Element[] = ['fire', 'water', 'nature', 'dark', 'light', 'neutral'];
const RARITIES_ALL: Rarity[] = ['Common', 'Uncommon', 'Rare', 'Epic', 'Legendary'];
...
const CardThumbnail = memo(({
    card, selected, onSelect, onOpen,
}: {
    card: CardFrameData & { id: string }; selected: boolean;
...
const CardListRow = memo(({
    ...
    <div
      ...
      ${selected
        ...
        onClick={() => onOpen(card.id)}
    >
        <button onClick={(e) => { e.stopPropagation(); onSelect(card.id); }}
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1">
                ...
                {(card.traits ?? []).slice(0, 2).map(t => (
                    <span key={t} className="text-[9px] text-slate-500 bg-white/[0.04] px-2 py-0.5 rounded-full border border-white/[0.06]">{t}</span>
                ))}
            </div>
        </div>
        <div className="flex items-center gap-4 text-xs text-slate-400 shrink-0">
            <span className="text-right"><span className="text-[9px] text-slate-600">هجوم </span>{card.attack}</span>
            <span className="text-right"><span className="text-[9px] text-slate-600">صحة </span>{card.hp}</span>
            <span className="text-right"><span className="text-[9px] text-slate-600">تكلفة </span>{card.cost}</span>
        </div>
    </div>
));
...
export const CollectionPage = memo(() => {
    ...
    const filtered = useMemo(() => {
        return COLLECTION
            ...
            .sort((a, b) => {
                if (sort === 'rarity') return (RARITY_ORDER[b.rarity ?? 'Common'] ?? 0) - (RARITY_ORDER[a.rarity ?? 'Common'] ?? 0);
        ...
        <div className="h-full flex flex-col overflow-hidden bg-[#070a14]">
            ...
            <div className="px-4 py-3 border-b border-white/[0.06] bg-[#0b0e1a] flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-1 bg-white/[0.04] border border-white/[0.07] rounded-xl px-2 py-1">
                    ...
                    <select value={filterElement} onChange={e => setFilterElement(e.target.value as any)}
                        ...
                        {ELEMENTS_ALL.map(el => <option key={el} value={el}>{el}</option>)}
                    </select>
                </div>
                ...
                <div className="flex items-center gap-1 bg-white/[0.04] border border-white/[0.07] rounded-xl px-2 py-1">
                    <select value={filterRarity} onChange={e => setFilterRarity(e.target.value as any)}
                        ...
                        {RARITIES_ALL.map(r => <option key={r} value={r}>{r}</option>)}
                    </select>
                </div>
                ...
                {/* Stats */}
                <span className="text-xs text-slate-500 shrink-0">{filtered.length} بطاقة</span>
            </div>
            ...
            {anySelected && (
                <div className="px-4 py-2 border-b border-purple-500/20 bg-purple-500/10 flex items-center gap-3 animate-slide-up">
                    ...
                    <span className="text-xs font-medium text-purple-300">تم تحديد {selected.size} بطاقة</span>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                        {filtered.map(card => (
                            <CardThumbnail
                                ...
                                onSelect={toggleSelect} onOpen={openCard}
                            />
                        ))}
                    </div>
                ) : (
