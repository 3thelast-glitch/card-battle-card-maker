      ...
      <aside className="w-64 shrink-0 border-r border-white/[0.06] bg-[#0b0e1a] flex flex-col overflow-hidden">
        ...
        <div className="p-3 border-b border-white/[0.06]">
          ...
          <div className="flex flex-wrap gap-1">
            <button
              onClick={() => updateConfig({ elementFilter: 'random' })}
            ...
            {ELEMENT_OPTS.map(({ e, label }) => (
              <button
                key={e}
                onClick={() => updateConfig({ elementFilter: e })}
                className={`px-2 py-1 rounded-lg text-[9px] font-medium border transition-all
        ...
        <div className="p-3">
          ...
          <div className="flex flex-wrap gap-1">
            <button
              onClick={() => updateConfig({ rarityFilter: 'random' })}
              className={`px-2 py-1 rounded-lg text-[9px] font-medium border transition-all
            ...
            {RARITY_OPTS.map(({ r, label }) => (
              <button
                key={r}
                onClick={() => updateConfig({ rarityFilter: r })}
                className={`px-2 py-1 rounded-lg text-[9px] font-medium border transition-all
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <div className="p-5 border-b border-white/[0.06] bg-black/20 flex flex-col gap-4 shrink-0">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-base font-black text-white">
                توليد بطاقات بالذكاء الاصطناعي
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                أدخل فكرة أو اختر موضوعًا، وسيُنشئ Gemini بطاقات احترافية لك
              </p>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <Sparkles
              size={15}
              className="absolute top-3 right-4 text-purple-400 pointer-events-none"
            />
          </div>
          ...
          {isGenerating && (
            <div className="w-full h-1 rounded-full overflow-hidden bg-white/[0.05]">
              <div
                ...
                style={{
                  background:
                    'linear-gradient(90deg, transparent, rgba(155,77,255,0.8), transparent)',
            ...
            <div className="h-full flex flex-col items-center justify-center gap-4 text-center">
              ...
              <div>
                <p className="text-slate-400 font-medium">
                  أدخل موضوعًا وانقر توليد
                </p>
                <p className="text-xs text-slate-600 mt-1">
                  يستخدم Gemini AI لإنشاء بطاقات ذات إحصائيات متوازنة
                </p>
              </div>
            </div>
          ) : isGenerating ? (
