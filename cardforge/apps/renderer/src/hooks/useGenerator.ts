  ...
  const generate = useCallback(
    async (
      ...
      try {
        ...
        if (start === -1 || end === -1)
          throw new Error('لم يُنتج الذكاء الاصطناعي نتيجة صالحة');
        ...
        const cards: GeneratedCard[] = parsed.slice(0, count).map((item) => ({
          ...
          element: (elFilter !== 'random'
            ? elFilter
            : (item.element ?? randomElement())) as Element,
          rarity: (rarFilter !== 'random'
            ? rarFilter
            : randomRarity()) as Rarity,
